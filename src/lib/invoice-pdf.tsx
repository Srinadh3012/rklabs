...
function drawRibbon(doc: jsPDF, text: string, x: number, y: number, w: number, h: number, color: [number, number, number]) {
  doc.setFillColor(color[0], color[1], color[2]);
  // The ribbon should have an angled cut on the right.
  // Triangle 1: Main rect from x to x + w - h
  doc.rect(x, y, w - h, h, 'F');
  // Triangle 2: Pointy bit on the right
...
export async function buildInvoicePdf({ shop, invoice, customer, repair, items }: InvoicePdfInput) {
  ...
  flushSync(() => {
    root.render(<QRCodeCanvas id={`temp-qr-${invoiceNo}`} value={verifyUrl} size={100} level="M" marginSize={1} />);
  });
  ...
  const drawHeader = (doc: jsPDF) => {
    ...
    doc.setFontSize(22);
    doc.setFont("helvetica", "bold");
    doc.text("TAX INVOICE", pw - marginX, 40, { align: "right" });
    
    // Invoice Badge (larger)
    doc.setDrawColor(orange[0], orange[1], orange[2]);
    ...
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(8);
    doc.text("INVOICE NO.", pw - marginX - 70, 68, { align: "center" });
    
    doc.setTextColor(orange[0], orange[1], orange[2]);
    doc.setFontSize(18);
    doc.text(invoiceNo, pw - marginX - 70, 95, { align: "center" });
    doc.setFont("helvetica", "normal");
    
    doc.addImage(barcodeDataUrl, "PNG", pw - marginX - 130, 115, 120, 30);
  ...
  drawRibbon(doc, "BILL TO / CUSTOMER", marginX - 5, currentY, 130, 14, teal);
  ...
  doc.text("Customer Name", labelX, currentY + 35); doc.text(":", colonX, currentY + 35);
  doc.text("Mobile No.", labelX, currentY + 55); doc.text(":", colonX, currentY + 55);
  doc.text("Email ID", labelX, currentY + 75); doc.text(":", colonX, currentY + 75);
  doc.text("Address", labelX, currentY + 95); doc.text(":", colonX, currentY + 95);
  ...
  // Table Label Ribbon
  drawRibbon(doc, "PARTICULARS / LINE ITEMS", marginX - 5, currentY, 150, 14, teal);
  
  const tableData = items.length > 0 ? items.map((item, idx) => [
    idx + 1,
    item.description,
    item.quantity,
  ...
  autoTable(doc, {
    ...
    columnStyles: {
      ...
      4: { cellWidth: 93, halign: 'right', textColor: teal, fontStyle: 'bold' }
    },
    didParseCell: (data) => {
  ...
  let remaining = footerY - currentY - 330;
  ...
  drawRibbon(doc, "PAYMENT STATUS", marginX - 5, currentY, 110, 14, orange);
  ...
  const statusColor = status === "PAID" ? [22, 163, 74] : status === "PARTIAL" ? [245, 158, 11] : [220, 38, 38];
  ...
  drawRibbon(doc, "INVOICE SUMMARY", totX - 5, currentY, 120, 14, teal);
  ...
  doc.setFontSize(8);
  doc.setFont("helvetica", "normal");
  const terms = shop?.terms?.length ? shop.terms : [
  ...
  doc.triangle(marginX + halfW - 55, currentY + 57, marginX + halfW - 32.5, currentY + 70, marginX + halfW - 10, currentY + 57, 'F');
  doc.setTextColor(255, 255, 255);
  doc.setFontSize(7);
  doc.setFont("helvetica", "bold");
