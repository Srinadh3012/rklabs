...
import { Calendar, Clock, CheckCircle2, MapPin, Phone, Globe, PenTool, User, Laptop, Wrench, Package, FileText, IndianRupee } from "lucide-react";
...
function drawRibbon(doc: jsPDF, text: string, x: number, y: number, w: number, h: number, color: [number, number, number], icon?: string) {
...
export async function buildIntakeReceiptPdf({ shop, invoice, customer, repair }: IntakeReceiptPdfInput) {
  ...
  flushSync(() => {
    root.render(<QRCodeCanvas id={`temp-qr-intake-${jobID}`} value={verifyUrl} size={100} level="M" marginSize={1} />);
  });
  ...
  doc.text(`${repair?.device_brand || ''} ${repair?.device_model || ''}`.trim() || 'N/A', marginX + 85, currentY + 50);
  ...
  doc.setTextColor(100, 100, 100);
  doc.setFontSize(8);
  doc.text("[ ] Charger     [ ] Battery     [ ] Bag     [ ] Box     [ ] Other", marginX + 15, currentY + 35);
  ...
  doc.triangle(totX + sumW - 55, currentY + 57, totX + sumW - 32.5, currentY + 70, totX + sumW - 10, currentY + 57, 'F');
  doc.setTextColor(255, 255, 255);
  doc.setFontSize(7);
  doc.setFont("helvetica", "bold");
  ...
  doc.setTextColor(teal[0], teal[1], teal[2]);
  doc.setFont("helvetica", "bold");
  doc.text("TECHNICIAN SIGNATURE", totX + 50 + (sumW - 65) / 2, currentY + 125, { align: "center" });
