...
export type AdvanceReceiptInput = {
  shop: { shop_name?: string | null; shop_address?: string | null; shop_phone?: string | null; gst_number?: string | null } | null;
  ticket_no: string;
  created_at: string;
  customer: { name?: string; phone?: string | null; address?: string | null } | null;
  device: { type?: string | null; brand?: string | null; model?: string | null; imei?: string | null } | null;
  issue: string;
...
export function buildAdvanceReceiptPdf(input: AdvanceReceiptInput) {
  ...
  doc.setDrawColor(200); doc.line(14, 44, 200, 44);

  doc.setFont("helvetica", "bold"); doc.text("Customer", 14, 52);
  ...
  doc.setFont("helvetica", "bold"); doc.text("Device", 120, 52);
  doc.setFont("helvetica", "normal");
