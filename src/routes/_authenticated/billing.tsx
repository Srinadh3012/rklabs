...
import { Plus, Trash2, FileDown, MessageCircle, Receipt as ReceiptIcon, IndianRupee, Send, Eye } from "lucide-react";
...
import { getInvoicesFn, createInvoiceFn, deleteInvoiceFn, getInvoiceItemsFn, createInvoiceItemsFn } from "@/lib/api/invoices";
...
type Invoice = {
  id: string; invoice_no: string; customer_id: string | null; repair_id: string | null;
  subtotal: number; discount: number; tax_rate: number; tax_amount: number; total: number;
  payment_method: string | null; payment_status: "unpaid" | "partial" | "paid"; created_at: string;
};
type Customer = { id: string; name: string; phone: string | null; whatsapp: string | null; address: string | null };
type Repair = { id: string; ticket_no: string; device_brand: string | null; device_model: string | null; imei: string | null };
type Profile = { id?: string; shop_name: string | null; shop_address: string | null; shop_phone: string | null; gst_number: string | null; gst_percent: number | null; wa_templates?: any };
...
function BillingPage() {
  ...
  const [items, setItems] = useState<LineItem[]>([{ description: "Repair charges", quantity: 1, unit_price: 0 }]);
  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      const payload: any = {
        ...
        subtotal, discount, tax_rate: gst, tax_amount: gstAmount, total,
        payment_method: paymentMode, payment_status: paymentStatus,
      };
      ...
      const rows = items.filter((i) => i.description).map((i) => ({
        invoice_id: inv.id,
        description: i.description, quantity: i.quantity, unit_price: i.unit_price, total_price: i.quantity * i.unit_price,
      }));
    ...
    onSuccess: async (inv) => {
      ...
      setDiscount(0); setCustomerId(""); setRepairId(""); setInvoiceNo("");
  ...
  const del = useMutation({
    mutationFn: async (id: string) => { await deleteInvoiceFn({ data: id }); },
  ...
  async function getPdfDoc(inv: Invoice) {
    ...
    return generatePdfFromHtml({
      type: "invoice", shop: profile ?? null, invoice: inv as any, customer: cust, repair: rep,
  ...
  async function uploadAndGetUrl(inv: Invoice): Promise<string> {
    const doc = await getPdfDoc(inv);
    doc.save(`${inv.invoice_no}.pdf`);
    toast.info("PDF downloaded! Please attach it to the WhatsApp chat when it opens.", { duration: 8000 });
  ...
  async function shareWhatsApp(inv: Invoice) {
    ...
    try {
      ...
      const msg = fillTemplate(tpl, { name: cust.name, invoice_no: inv.invoice_no, amount: inrPrecise(inv.total), shop: profile?.shop_name ?? "RK Labs", link });
  ...
  async function paymentReminder(inv: Invoice) {
    ...
    const msg = fillTemplate(tpl, { name: cust.name, invoice_no: inv.invoice_no, amount: inrPrecise(inv.total), shop: profile?.shop_name ?? "RK Labs" });
    wa.send({
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      <header className="flex flex-wrap items-center justify-between gap-4 border-b border-white/10 pb-6">
        <div className="space-y-1.5">
          ...
          <p className="text-sm text-slate-400">Manage invoices, payments, GST and customer billing</p>
        </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <select value={customerId} onChange={(e) => setCustomerId(e.target.value)} className="h-9 w-full rounded-md border border-input bg-input/40 px-3 text-sm">
                ...
                <div className="space-y-1.5">
                  ...
                  <select value={repairId} onChange={(e) => setRepairId(e.target.value)} className="h-9 w-full rounded-md border border-input bg-input/40 px-3 text-sm">
                    <option value="">— None —</option>
                    {repairs.map((r) => <option key={r.id} value={r.id}>{r.ticket_no}</option>)}
                  </select>
                </div>
              </div>

              <div className="space-y-1.5">
                <Label>Invoice number <span className="text-xs text-muted-foreground">(letters + numbers — leave blank to auto-generate)</span></Label>
                <Input
                    ...
                    <Button
                      ...
                      onClick={() => {
                        setItems((prev) => {
                          ...
                          const newItem = { description: q.label, quantity: 1, unit_price: q.price };
              ...
              <div className="space-y-2">
                ...
                {items.map((it, idx) => (
                  <div key={idx} className="grid grid-cols-[1fr_70px_110px_110px_40px] gap-2">
                    <Input placeholder="Description" value={it.description} onChange={(e) => setItems(items.map((x, i) => i === idx ? { ...x, description: e.target.value } : x))} />
                    <Input type="number" min={0} value={it.quantity} onChange={(e) => setItems(items.map((x, i) => i === idx ? { ...x, quantity: Number(e.target.value) } : x))} />
                    <Input type="number" step="0.01" placeholder="Unit ₹" value={it.unit_price} onChange={(e) => setItems(items.map((x, i) => i === idx ? { ...x, unit_price: Number(e.target.value) } : x))} />
                    <div className="grid h-9 place-items-center rounded-md border border-white/10 bg-white/5 text-sm">{inr(it.quantity * it.unit_price)}</div>
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className="space-y-1.5"><Label>GST %</Label><Input type="number" value={gst} onChange={(e) => setGst(Number(e.target.value))} /></div>
                <div className="space-y-1.5">
                  <Label>Payment mode</Label>
                  <select value={paymentMode} onChange={(e) => setPaymentMode(e.target.value)} className="h-9 w-full rounded-md border border-input bg-input/40 px-3 text-sm">
                    <option>Cash</option><option>UPI</option><option>Card</option><option>Bank Transfer</option>
                  </select>
                </div>
                <div className="space-y-1.5">
                  <Label>Status</Label>
                  <select value={paymentStatus} onChange={(e) => setPaymentStatus(e.target.value as any)} className="h-9 w-full rounded-md border border-input bg-input/40 px-3 text-sm">
              ...
              <label className="flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-xs">
                ...
                <span>
                  ...
                  {!customerId && <span className="ml-1 text-muted-foreground">(select a customer to enable)</span>}
                </span>
              </label>
            </div>
            <DialogFooter>
              <Button onClick={() => create.mutate()} disabled={create.isPending} style={{ background: "var(--gradient-primary)", color: "oklch(0.12 0.02 250)" }}>
                Create invoice
              </Button>
            </DialogFooter>
          </DialogContent>
        ...
        <Dialog open={!!previewPdfUrl} onOpenChange={(o) => !o && setPreviewPdfUrl(null)}>
          <DialogContent className="glass-strong max-h-[95vh] w-[95vw] max-w-5xl overflow-hidden p-0 flex flex-col border-white/20">
            <DialogHeader className="p-4 border-b border-white/10 shrink-0 bg-slate-900/50">
              <DialogTitle className="text-lg font-semibold tracking-tight">Invoice Preview</DialogTitle>
            </DialogHeader>
            <div className="flex-1 min-h-[75vh] w-full bg-[#323639] relative">
              {previewPdfUrl && (
                <iframe src={previewPdfUrl} className="absolute inset-0 w-full h-full border-0 bg-white" title="PDF Preview" />
              )}
            </div>
            <DialogFooter className="p-4 border-t border-white/10 shrink-0 bg-slate-900/50">
              <Button variant="outline" className="px-6" onClick={() => setPreviewPdfUrl(null)}>Close</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <Stat icon={ReceiptIcon} label="Total Invoices" value={String(invoices.length)} type="primary" />
        <Stat icon={IndianRupee} label="Collected Revenue" value={inr(totalRevenue)} type="success" />
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-white/5">
              {isLoading && <tr><td colSpan={6} className="py-16 text-center text-muted-foreground">Loading invoices...</td></tr>}
              {!isLoading && invoices.length === 0 && <tr><td colSpan={6} className="py-16 text-center text-muted-foreground">No invoices yet. Create your first invoice to start managing billing and GST records.</td></tr>}
              {invoices.map((inv) => {
                ...
                return (
                  <tr key={inv.id} className="group transition-colors hover:bg-white/[0.02]">
                    <td className="px-4 py-3.5 font-mono text-xs font-semibold text-slate-200">{inv.invoice_no}</td>
                    <td className="px-4 py-3.5 font-medium text-slate-300">{c?.name ?? <span className="text-slate-500">Walk-in</span>}</td>
                    <td className="px-4 py-3.5 text-slate-400">{fmtDate(inv.created_at)}</td>
                    <td className="px-4 py-3.5 text-right font-semibold text-slate-200">{inr(inv.total)}</td>
                    <td className="px-4 py-3.5">
                      <Badge variant="outline" className={cn("px-2 py-0.5 text-[10px] uppercase tracking-wider font-bold rounded-md border", statusColors[inv.payment_status])}>
                        {inv.payment_status}
                      </Badge>
                    </td>
                    <td className="px-4 py-3.5 text-right">
                      <div className="flex items-center justify-end gap-1 opacity-80 group-hover:opacity-100 transition-opacity">
                        <Button size="icon" variant="ghost" className="h-8 w-8 hover:bg-white/10 hover:text-white" title="Preview PDF" onClick={() => previewPdf(inv)}><Eye className="h-4 w-4" /></Button>
                        <Button size="icon" variant="ghost" className="h-8 w-8 hover:bg-white/10 hover:text-white" title="Download PDF" onClick={() => downloadPdf(inv)}><FileDown className="h-4 w-4" /></Button>
                        <Button size="icon" variant="ghost" className="h-8 w-8 hover:bg-[var(--neon)]/20 hover:text-[var(--neon)] text-[var(--neon)]" title="Send PDF on WhatsApp" onClick={() => shareWhatsApp(inv)}><Send className="h-4 w-4" /></Button>
                        {inv.payment_status !== "paid" && (
                          <Button size="icon" variant="ghost" className="h-8 w-8 hover:bg-amber-500/20 hover:text-amber-400 text-amber-300" title="Payment reminder" onClick={() => paymentReminder(inv)}><MessageCircle className="h-4 w-4" /></Button>
                        )}
                        <Button size="icon" variant="ghost" className="h-8 w-8 hover:bg-red-500/20 hover:text-red-400 text-red-400/80" title="Delete Invoice" onClick={() => { if (confirm(`Delete ${inv.invoice_no}?`)) del.mutate(inv.id); }}><Trash2 className="h-4 w-4" /></Button>
                      </div>
                    </td>
                  </tr>
                );
...
function Stat({ icon: Icon, label, value, type }: { icon: any; label: string; value: string; type: "primary" | "success" | "warning" }) {
  const colors = {
