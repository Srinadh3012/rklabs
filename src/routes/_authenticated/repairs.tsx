...
import { Plus, Search, Edit, Trash2, MessageCircle, Sparkles, Printer, ClipboardList, Check, Languages, Loader2, UserPlus, CalendarPlus, CalendarClock, CalendarX, X } from "lucide-react";
...
import { getRepairsFn, createRepairFn, updateRepairFn, deleteRepairFn, getRepairNotesFn, createRepairNoteFn, updateRepairNoteFn, getAppointmentEventsFn, createAppointmentEventFn, getWaLogsFn } from "@/lib/api/repairs";
...
type Repair = {
  id: string; ticket_no: string; customer_id: string | null;
  device_type: string | null; device_brand: string | null; device_model: string | null; imei: string | null;
  issue: string; status: string; technician_name: string | null; technician_notes: string | null;
  estimated_completion: string | null; estimated_cost: number | null; final_cost: number | null;
  appointment_at: string | null;
  created_at: string; completed_at: string | null; delivered_at: string | null; assigned_at: string | null;
};
type Customer = { id: string; name: string; whatsapp: string | null; phone: string | null; address: string | null };
type RepairNote = { id: string; repair_id: string; technician_name: string | null; note: string; task_done: boolean; completed_at: string | null; created_at: string };
type Profile = { shop_name: string | null; shop_address: string | null; shop_phone: string | null; gst_number: string | null; wa_templates?: any };
...
const EMPTY_FORM: FormState = {
  customer_id: "", device_type: "Mobile", device_brand: "", device_model: "", imei: "",
  issue: "", status: "received", estimated_completion: "", estimated_cost: 0,
  advance_amount: 0, payment_mode: "Cash", technician_notes: "",
  appointment_date: "", appointment_time: "", auto_send_wa: true, customer_mobile: "",
};
...
function RepairsPage() {
  ...
  const custSuggestions = useMemo(() => {
    ...
    return customers
      .filter((c) => c.name.toLowerCase().includes(q) || c.phone?.includes(q) || c.whatsapp?.includes(q))
      .slice(0, 6);
      ...
      if (isNew && form.advance_amount > 0) {
        ...
        try {
          const doc = buildAdvanceReceiptPdf({
            ...
            device: { type: form.device_type, brand: form.device_brand, model: form.device_model, imei: form.imei },
            issue: form.issue,
          ...
          if (cust?.whatsapp) {
            ...
            const tpl = profile?.wa_templates?.advance ??
  ...
  const updateStatus = useMutation({
    ...
    onSuccess: ({ status, id }) => {
      ...
      if (tpl && c?.whatsapp && r) {
        const msg = fillTemplate(tpl, {
          name: c.name, device: [r.device_brand, r.device_model].filter(Boolean).join(" ") || r.device_type || "device",
          ticket: r.ticket_no, shop: profile?.shop_name ?? "RK Labs",
        });
  ...
  const del = useMutation({
    mutationFn: async (id: string) => { await deleteRepairFn({ data: id }); },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["repairs"] }); toast.success("Deleted"); },
  });
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      <header className="flex flex-wrap items-center justify-between gap-4 border-b border-white/10 pb-6">
        <div className="space-y-1.5">
          ...
          <p className="text-sm text-slate-400">{repairs.length} tickets · workflow tracking · WhatsApp receipts</p>
        </div>
        <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) resetForm(); }}>
          ...
          <DialogContent className="glass-strong max-h-[90vh] max-w-2xl overflow-y-auto custom-scrollbar border-white/10 shadow-2xl">
            <DialogHeader className="border-b border-white/10 pb-4 mb-4"><DialogTitle className="text-xl tracking-tight">{editing ? `Edit ${editing.ticket_no}` : "New repair ticket"}</DialogTitle></DialogHeader>
            <form onSubmit={(e) => { e.preventDefault(); save.mutate(); }} className="space-y-4">
                ...
                <div className="relative">
                  <div className="flex gap-2">
                    <Input
                      ...
                      value={custSearch}
                      onChange={(e) => { setCustSearch(e.target.value); if (form.customer_id) setForm((f) => ({ ...f, customer_id: "" })); }}
                      className="bg-black/20 border-white/10 focus:border-cyan-500/50"
                    />
                    <Button type="button" variant="outline" size="icon" className="shrink-0 border-white/10 bg-white/5 hover:bg-white/10 hover:text-cyan-400" onClick={() => setNewCustOpen(true)} title="Add new customer">
                    ...
                    <div className="absolute z-20 mt-1 max-h-60 w-full overflow-y-auto rounded-xl border border-white/10 bg-[#0f172a] shadow-xl custom-scrollbar">
                      {custSuggestions.map((c) => (
                        <button
                          ...
                          onClick={() => { setForm((f) => ({ ...f, customer_id: c.id, customer_mobile: f.customer_mobile || c.phone || c.whatsapp || "" })); setCustSearch(c.name); }}
                        >
                          <span className="font-medium text-slate-200">{c.name}</span>
                          <span className="text-xs text-slate-400">{c.phone ?? c.whatsapp ?? "—"}{c.address ? ` · ${c.address}` : ""}</span>
                        </button>
                      ))}
                    </div>
                  )}
                ...
                {selectedCustomer && (
                  <div className="rounded-xl border border-cyan-500/20 bg-cyan-500/5 p-3 text-xs mt-2 shadow-sm">
                    ...
                    <div className="text-cyan-400/70 mt-1">
                      ...
                      {selectedCustomer.whatsapp && <span className="mr-2">💬 {selectedCustomer.whatsapp}</span>}
                      {selectedCustomer.address && <div className="mt-1">📍 {selectedCustomer.address}</div>}
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  className="bg-black/20 border-white/10 focus:border-cyan-500/50"
                />
                <div className="text-[11px] text-slate-400">Saved to the selected customer's record if their phone is empty.</div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  >
                    {DEVICE_TYPES.map((t) => <option key={t} value={t} className="bg-[#0f172a]">{t}</option>)}
                  </select>
                </div>
                <FieldC label="Brand" value={form.device_brand} onChange={(v) => setForm((f) => ({ ...f, device_brand: v }))} className="bg-black/20 border-white/10 focus:border-cyan-500/50" />
                <FieldC label="Model" value={form.device_model} onChange={(v) => setForm((f) => ({ ...f, device_model: v }))} className="bg-black/20 border-white/10 focus:border-cyan-500/50" />
                <FieldC label="IMEI / Serial" value={form.imei} onChange={(v) => setForm((f) => ({ ...f, imei: v }))} className="bg-black/20 border-white/10 focus:border-cyan-500/50" />
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    className="h-10 w-full rounded-md border border-white/10 bg-black/20 px-3 text-sm focus:border-cyan-500/50 outline-none"
                  >
                    {STATUSES.map((s) => <option key={s.v} value={s.v} className="bg-[#0f172a]">{s.label}</option>)}
                  </select>
                </div>
                <FieldC label="Estimated completion" type="date" value={form.estimated_completion} onChange={(v) => setForm((f) => ({ ...f, estimated_completion: v }))} className="bg-black/20 border-white/10 focus:border-cyan-500/50" />
                <FieldC label="Estimated cost (₹)" type="number" step="0.01" value={String(form.estimated_cost)} onChange={(v) => setForm((f) => ({ ...f, estimated_cost: Number(v || 0) }))} className="bg-black/20 border-white/10 focus:border-cyan-500/50" />
              </div>

              <div className="rounded-xl border border-white/10 bg-white/5 p-4 space-y-3 shadow-inner">
                <div className="text-xs font-bold uppercase tracking-wider text-cyan-400">Appointment slot (optional)</div>
                <div className="grid grid-cols-2 gap-4">
                  <FieldC
                  ...
                  <div className="space-y-1.5">
                    ...
                    >
                      <option value="" className="bg-[#0f172a]">Select a slot…</option>
                      {TIME_SLOTS.map((t) => <option key={t} value={t} className="bg-[#0f172a]">{t}</option>)}
                    </select>
                  </div>
                </div>
                <div className="text-[11px] text-slate-400">Pick when the customer will drop off the device. Saved with the ticket.</div>
              </div>

              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <Label className="text-slate-300">Issue description</Label>
                  <Button type="button" size="sm" variant="ghost" className="h-8 text-cyan-400 hover:text-cyan-300 hover:bg-cyan-400/10" onClick={handleTranslate} disabled={translating || !form.issue.trim()}>
                    {translating ? <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> : <Languages className="mr-1.5 h-3.5 w-3.5" />}
                ...
                {suggestion && (
                  <div className="flex gap-3 rounded-xl border border-cyan-500/20 bg-cyan-500/5 p-4 text-sm mt-2 shadow-sm">
                    ...
                    <div className="text-slate-300"><span className="font-semibold text-cyan-400">AI suggestion:</span> {suggestion}</div>
                  </div>
                )}
              </div>
              ...
              {!editing && (
                <div className="rounded-xl border border-white/10 bg-white/5 p-4 space-y-4 shadow-inner">
                  <div className="text-xs font-bold uppercase tracking-wider text-emerald-400">Advance payment (optional)</div>
                  <div className="grid grid-cols-3 gap-4">
                    <FieldC label="Advance (₹)" type="number" step="0.01" value={String(form.advance_amount)} onChange={(v) => setForm((f) => ({ ...f, advance_amount: Number(v || 0) }))} className="bg-black/20 border-white/10" />
                    <div className="space-y-1.5">
                      ...
                      >
                        <option className="bg-[#0f172a]">Cash</option><option className="bg-[#0f172a]">UPI</option><option className="bg-[#0f172a]">Card</option><option className="bg-[#0f172a]">Bank Transfer</option>
                      </select>
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-slate-300">Balance due</Label>
                      <div className="grid h-10 place-items-center rounded-md border border-white/10 bg-black/20 text-sm font-bold text-emerald-400">{inr(balance)}</div>
                    </div>
                  </div>
                  <label className="flex items-center gap-3 rounded-lg border border-white/10 bg-black/20 px-4 py-3 text-sm cursor-pointer hover:bg-white/5 transition-colors">
                    ...
                    />
                    <span className="text-slate-300 text-sm">One-click WhatsApp — auto-send advance receipt to customer</span>
                  </label>
                </div>
              )}
              ...
              <div className="space-y-1.5 pt-2">
                ...
                <Textarea value={form.technician_notes} onChange={(e) => setForm((f) => ({ ...f, technician_notes: e.target.value }))} rows={2} className="bg-black/20 border-white/10 resize-none min-h-[60px]" />
              </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-[#0f172a]/80 backdrop-blur-xl p-4 shadow-lg">
        <div className="mb-4 flex flex-wrap gap-3">
          <div className="relative flex-1 min-w-[220px]">
            ...
            <Input placeholder="Search ticket, IMEI, brand, customer…" value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 h-10 bg-black/20 border-white/10" />
          </div>
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            <SelectTrigger className="h-10 w-48 bg-black/20 border-white/10 text-slate-200"><SelectValue /></SelectTrigger>
        ...
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-sm whitespace-nowrap">
            ...
            <tbody>
              {isLoading && <tr><td colSpan={8} className="py-12 text-center text-slate-500">Loading…</td></tr>}
              {!isLoading && filtered.length === 0 && <tr><td colSpan={8} className="py-12 text-center text-slate-500">No repair tickets.</td></tr>}
              {filtered.map((r) => {
                const c = r.customer_id ? custMap.get(r.customer_id) : null;
                return (
                  <tr key={r.id} className="group border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                    <td className="px-4 py-3 font-mono text-xs font-semibold">
                      <button className="text-cyan-400 hover:text-cyan-300 hover:underline transition-colors" onClick={() => setDetail(r)}>{r.ticket_no}</button>
                    </td>
                    <td className="px-4 py-3 text-slate-200">{c?.name ?? <span className="text-slate-500">—</span>}</td>
                    <td className="px-4 py-3 text-slate-400 truncate max-w-[200px]" title={[r.device_brand, r.device_model].filter(Boolean).join(" ") || r.device_type || ""}>
                      {[r.device_brand, r.device_model].filter(Boolean).join(" ") || r.device_type || "—"}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      ...
                      >
                        {STATUSES.map((s) => <option key={s.v} value={s.v} className="bg-[#0f172a] text-slate-200 normal-case">{s.label}</option>)}
                      </select>
                    </td>
                    <td className="px-4 py-3 text-right font-medium text-slate-200">{inr(r.estimated_cost)}</td>
                    <td className="px-4 py-3 text-slate-400">{fmtDate(r.estimated_completion)}</td>
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Button size="icon" variant="ghost" className="h-8 w-8 text-slate-400 hover:text-slate-100 hover:bg-white/10" aria-label={`Open details for ${r.ticket_no}`} title="Open details" onClick={() => setDetail(r)}><ClipboardList className="h-4 w-4" /></Button>
                        {c?.whatsapp && (
                          <Button
                            size="icon"
                            variant="ghost"
                        ...
                        <Button size="icon" variant="ghost" className="h-8 w-8 text-slate-400 hover:text-cyan-400 hover:bg-cyan-400/10" aria-label={`Edit ${r.ticket_no}`} onClick={() => loadEditing(r)}><Edit className="h-4 w-4" /></Button>
                        <Button size="icon" variant="ghost" className="h-8 w-8 text-slate-400 hover:text-red-400 hover:bg-red-400/10" aria-label={`Delete ${r.ticket_no}`} onClick={() => { if (confirm(`Delete ${r.ticket_no}?`)) del.mutate(r.id); }}><Trash2 className="h-4 w-4" /></Button>
                      </div>
                    </td>
                  </tr>
                );
...
function FieldC({ label, value, onChange, ...props }: { label: string; value: string; onChange: (v: string) => void } & Omit<React.InputHTMLAttributes<HTMLInputElement>, "value" | "onChange">) {
  return (
    <div className="space-y-1.5">
      <Label>{label}</Label>
...
function QuickCustomerDialog({ onClose, onCreated }: { onClose: () => void; onCreated: (c: Customer) => void }) {
  ...
  async function submit() {
    ...
    try {
      ...
      const data = await createCustomerFn({ data: {
        name, phone: phone || null, whatsapp: whatsapp || phone || null, address: address || null,
      } });
      ...
      <DialogContent className="glass-strong max-w-md">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5"><Label>WhatsApp</Label><Input value={whatsapp} onChange={(e) => setWhatsapp(e.target.value)} placeholder="Same as phone if empty" /></div>
          </div>
          <div className="space-y-1.5"><Label>Address</Label><Input value={address} onChange={(e) => setAddress(e.target.value)} /></div>
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={onClose}>Cancel</Button>
          <Button onClick={submit} disabled={saving} style={{ background: "var(--gradient-primary)", color: "oklch(0.12 0.02 250)" }}>
...
function RepairDetailDialog({ repair, customer, profile, onClose }: { repair: Repair; customer: Customer | null; profile: Profile | null; onClose: () => void }) {
  const qc = useQueryClient();
  ...
  const addNote = useMutation({
    mutationFn: async () => {
      ...
      await createRepairNoteFn({ data: {
        repair_id: repair.id, note: newNote, technician_name: newTech || repair.technician_name || undefined,
      } });
    },
    onSuccess: () => { setNewNote(""); qc.invalidateQueries({ queryKey: ["repair-notes", repair.id] }); toast.success("Note added"); },
    onError: (e: any) => toast.error(e.message),
  });

  const toggleDone = useMutation({
    mutationFn: async (n: RepairNote) => {
      await updateRepairNoteFn({ data: { id: n.id, data: { task_done: !n.task_done, completed_at: !n.task_done ? new Date().toISOString() : null } } });
    },
      ...
      <DialogContent className="glass-strong max-h-[90vh] max-w-3xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center justify-between gap-3">
            ...
            <Button size="sm" variant="outline" onClick={printReceipt}><Printer className="mr-2 h-4 w-4" /> Print receipt</Button>
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="glass space-y-1 rounded-xl p-4 text-sm">
            ...
            <div className="font-semibold">{[repair.device_brand, repair.device_model].filter(Boolean).join(" ") || "—"}</div>
        ...
        <div className="glass rounded-xl p-4">
          <div className="mb-3 text-xs uppercase tracking-wider text-muted-foreground">Workflow timeline</div>
          <ol className="space-y-2 text-sm">
            <TimelineItem label="Received" at={repair.created_at} />
            {repair.appointment_at && <TimelineItem label="Appointment scheduled" at={repair.appointment_at} accent />}
            <TimelineItem label={`Assigned to ${repair.technician_name ?? "technician"}`} at={repair.assigned_at} />
        ...
        <div className="glass rounded-xl p-4">
          <div className="mb-3 flex items-center justify-between">
            <div className="text-xs uppercase tracking-wider text-muted-foreground">Technician notes & tasks</div>
            <Badge variant="outline" className="border-white/20">{notes.filter((n) => n.task_done).length}/{notes.length} done</Badge>
          </div>
          <div className="space-y-2">
            {notes.length === 0 && <div className="text-sm text-muted-foreground">No notes yet.</div>}
            {notes.map((n) => (
              <div key={n.id} className="flex items-start gap-3 rounded-lg border border-white/5 bg-white/5 p-3 text-sm">
                <button onClick={() => toggleDone.mutate(n)} className={"mt-0.5 grid h-5 w-5 place-items-center rounded border " + (n.task_done ? "border-emerald-500 bg-emerald-500/20 text-emerald-300" : "border-white/20")}>
                  {n.task_done && <Check className="h-3 w-3" />}
                </button>
                <div className="flex-1">
                  <div className={n.task_done ? "line-through text-muted-foreground" : ""}>{n.note}</div>
                  <div className="mt-1 text-[11px] text-muted-foreground">{n.technician_name ?? "—"} · {fmtDateTime(n.created_at)}{n.task_done && n.completed_at && ` · done ${fmtDateTime(n.completed_at)}`}</div>
                </div>
              </div>
            ))}
          </div>
          <div className="mt-3 grid grid-cols-[180px_1fr_auto] gap-2">
            <Input placeholder="Technician name" value={newTech} onChange={(e) => setNewTech(e.target.value)} />
            <Input placeholder="Add a note / task…" value={newNote} onChange={(e) => setNewNote(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addNote.mutate()} />
            <Button onClick={() => addNote.mutate()} disabled={addNote.isPending}>Add</Button>
          </div>
        </div>
...
function TimelineItem({ label, at, accent }: { label: string; at: string | null; accent?: boolean }) {
  const dotColor = accent && at ? "bg-[var(--neon)] shadow-[0_0_8px_var(--neon)]" : at ? "bg-[var(--neon)] shadow-[0_0_8px_var(--neon)]" : "bg-white/15";
  return (
    <li className="flex items-center gap-3">
      <div className={"h-2.5 w-2.5 rounded-full " + dotColor} />
      <span className={at ? (accent ? "text-[var(--neon)] font-medium" : "") : "text-muted-foreground"}>{label}</span>
      <span className="ml-auto text-xs text-muted-foreground">{at ? fmtDateTime(at) : "Pending"}</span>
    </li>
  );
}

type ApptEvent = { id: string; action: "scheduled" | "rescheduled" | "cancelled"; previous_at: string | null; new_at: string | null; note: string | null; created_at: string };

function AppointmentPanel({ repair, customer, profile }: { repair: Repair; customer: Customer | null; profile: Profile | null }) {
  const qc = useQueryClient();
  const wa = useWaSender();
  ...
  const update = useMutation({
    mutationFn: async (next: string | null) => {
      ...
      const action: "scheduled" | "rescheduled" | "cancelled" = next
        ? previous_at ? "rescheduled" : "scheduled"
        : "cancelled";
      const { error } = await updateRepairFn({ data: { id: repair.id, data: { appointment_at: next } } }) as any;
    ...
    <div className="glass rounded-xl border border-[var(--neon)]/20 p-4">
          ...
          {calEvent && (
            <>
              <a href={googleCalendarUrl(calEvent)} target="_blank" rel="noreferrer">
                <Button size="sm" variant="outline" className="glass"><CalendarPlus className="mr-2 h-4 w-4" /> Google</Button>
              </a>
              <Button size="sm" variant="outline" className="glass" onClick={() => downloadIcs(calEvent, `${repair.ticket_no}.ics`)}>
                <CalendarPlus className="mr-2 h-4 w-4" /> .ics
              </Button>
            </>
          )}
          <Button size="sm" variant="outline" className="glass" onClick={() => setEditing((v) => !v)}>
            <Edit className="mr-2 h-4 w-4" /> {repair.appointment_at ? "Reschedule" : "Schedule"}
          </Button>
          {repair.appointment_at && (
            <Button size="sm" variant="outline" className="glass border-red-500/40 text-red-300 hover:bg-red-500/10"
              onClick={() => { if (confirm("Cancel this appointment?")) update.mutate(null); }} disabled={update.isPending}>
      ...
      {editing && (
        <div className="mt-3 grid grid-cols-[1fr_1fr_auto_auto] gap-2">
          ...
          <select value={time} onChange={(e) => setTime(e.target.value)} className="h-9 rounded-md border border-input bg-input/40 px-3 text-sm">
            {TIME_SLOTS.map((t) => <option key={t} value={t}>{t}</option>)}
          </select>
          <Button size="sm" onClick={saveNew} disabled={update.isPending} style={{ background: "var(--gradient-primary)", color: "oklch(0.12 0.02 250)" }}>Save</Button>
      ...
      <div className="mt-4">
        ...
        {historyQ.isLoading ? (
          ...
          <ul className="space-y-2">
            {events.map((ev) => (
              <li key={ev.id} className="flex flex-wrap items-center gap-2 rounded-lg border border-white/5 bg-white/[0.02] px-3 py-2 text-sm">
                <span className={`rounded-full border px-2 py-0.5 text-xs capitalize ${actionBadge[ev.action]}`}>{ev.action}</span>
                <span className="text-muted-foreground">
                  {ev.action === "cancelled"
                    ? ev.previous_at ? `was ${fmtDateTime(ev.previous_at)}` : "—"
                    : ev.action === "rescheduled"
                      ? `${ev.previous_at ? fmtDateTime(ev.previous_at) : "—"} → ${ev.new_at ? fmtDateTime(ev.new_at) : "—"}`
                      : ev.new_at ? fmtDateTime(ev.new_at) : "—"}
                </span>
                <span className="ml-auto text-xs text-muted-foreground">{fmtDateTime(ev.created_at)}</span>
              </li>
            ))}
          </ul>
        )}
...
type WaLog = { id: string; kind: string; recipient_name: string | null; phone: string | null; message: string; status: string; error: string | null; created_at: string };
...
function WaLogPanel({ repairId }: { repairId: string }) {
  ...
  return (
    <div className="glass rounded-xl p-4">
      <div className="mb-3 flex items-center justify-between">
        <div className="text-xs uppercase tracking-wider text-muted-foreground">WhatsApp send log</div>
        <Badge variant="outline" className="border-white/20">{logs.length} entries</Badge>
      </div>
      {isLoading ? (
        <div className="text-sm text-muted-foreground">Loading…</div>
      ) : logs.length === 0 ? (
        <div className="text-sm text-muted-foreground">No WhatsApp messages sent for this ticket yet.</div>
      ) : (
        <div className="space-y-2">
          {logs.map((l) => (
            <div key={l.id} className="rounded-lg border border-white/5 bg-white/5 p-3 text-xs">
              <div className="flex flex-wrap items-center gap-2">
                <Badge variant="outline" className={statusClass[l.status] ?? "border-white/20"}>{l.status}</Badge>
                <span className="font-semibold capitalize">{l.kind.replace(/_/g, " ")}</span>
                <span className="text-muted-foreground">→ {l.recipient_name ?? l.phone ?? "—"}</span>
                <span className="ml-auto text-muted-foreground">{fmtDateTime(l.created_at)}</span>
              </div>
              <div className="mt-2 whitespace-pre-wrap text-muted-foreground line-clamp-3">{l.message}</div>
